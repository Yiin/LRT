...
export class AppService {
    constructor(
        @Inject('axios') private axios: typeof Axios,
    ) {}
    ...
    private findCurrentShows(html): CurrentShow[] {
        ...
        return nodes.toArray().map((node, key) => ({
            key,
            logo: $(node).find('.logo__svg').toString(),
            title: $(node).find('.channel-item__title').text(),
            time: $(node).find('.data-block__text').text(),
            progress: $(node).find('.live-program__progress').css('width'),
            link: $(node).find('.channel-item__link').attr('href'),
        }));
    }
